...
const templatesData = [
  {
    templateName: "AURUMRISE_FOLLOWUP_1",
  ...
  {
    templateName: "AURUMRISE_FOLLOWUP_2",
  ...
  {
    templateName: "AURUMRISE_FOLLOWUP_3",
...
async function createTemplate(t) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.log(`Created template "${t.templateName}": ID ${data.id}`);
    } else {
      const err = await res.json();
      console.error(`Error creating template "${t.templateName}":`, err);
    }
  } catch (e) {
    console.error(`Failed to create ${t.templateName}:`, e.message);
  }
}
...
async function run() {
  ...
  console.log("Creation script execution complete.");
}
