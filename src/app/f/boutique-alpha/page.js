...
export default function BoutiqueAlphaLanding() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      window.location.href = '/f/boutique-alpha/bridge';
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#0a0a0c] text-white font-sans selection:bg-[#d4af37] selection:text-black">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-20 flex flex-col items-center text-center">
        ...
        <h1 className="text-5xl md:text-6xl font-extrabold leading-tight mb-6 bg-clip-text text-transparent bg-gradient-to-b from-white to-gray-400">
          Join Our Exclusive Downline Building Protocol
        </h1>
        
        <p className="text-xl text-gray-400 mb-12 max-w-2xl leading-relaxed">
          The Alpha Group uses a highly guarded "Queue-Based Rotator" to guarantee placement for our members. Discover how our unique ecosystem forces growth.
        </p>
        ...
        <div className="w-full max-w-md bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-xl shadow-[0_40px_100px_-20px_rgba(0,0,0,0.7)]">
          <div className="flex flex-col gap-4 text-left mb-8">
            <div className="flex items-start gap-3">
              ...
              <p className="text-gray-300"><strong>Private Rotator Access:</strong> Skip the line and get plugged into the team's automated spillover engine.</p>
            </div>
            <div className="flex items-start gap-3">
              <Users className="w-6 h-6 text-[#d4af37] mt-1" />
              <p className="text-gray-300"><strong>Guaranteed Placement:</strong> Our unique algorithm fills your downline sequentially.</p>
            </div>
            <div className="flex items-start gap-3">
              <Zap className="w-6 h-6 text-[#d4af37] mt-1" />
              <p className="text-gray-300"><strong>Instant Activation:</strong> Start leveraging the Aurum system within 15 minutes.</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            >
              {isProcessing ? (
                <><Loader2 className="w-5 h-5 animate-spin" /> Securing Spot...</>
              ) : (
                <>Unlock Invitation <ArrowRight className="w-5 h-5" /></>
              )}
            </button>
          </form>
