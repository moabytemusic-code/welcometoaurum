...
export default function AdminFreemiumPayments() {
  ...
  const fetchPayments = async () => {
    try {
      const res = await fetch('/api/admin/freemium-payments');
      if (!res.ok) {
        if (res.status === 401) {
          router.push('/admin/login');
          return;
        }
        throw new Error('Failed to fetch freemium payment records');
      }
  ...
  if (loading) {
    return (
      <div style={{
        ...
        <h2>Loading freemium verification panel...</h2>
      </div>
    );
  }
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div>
            <h1 style={{ fontSize: '28px', fontWeight: '900', margin: 0, letterSpacing: '-0.5px' }}>Freemium Upgrades Verification</h1>
            <p style={{ color: 'rgba(255,255,255,0.4)', margin: '4px 0 0 0', fontSize: '14px' }}>Review and approve checkout payments from Education Portal users.</p>
          </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px', textAlign: 'left' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.4)' }}>
                    ...
                    <th style={{ padding: '12px 16px' }}>Prospect Email</th>
                    <th style={{ padding: '12px 16px' }}>Prospect Name</th>
                  ...
                  {pendingPayments.map((p) => (
                    <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                      ...
                      <td style={{ padding: '16px' }}>
                        <strong style={{ display: 'block' }}>{p.prospect?.email || 'Unknown'}</strong>
                      </td>
                      <td style={{ padding: '16px', fontWeight: 'bold' }}>{p.prospect?.first_name || 'N/A'}</td>
                      <td style={{ padding: '16px', fontFamily: 'monospace', fontSize: '12px', wordBreak: 'break-all', maxWidth: '240px' }}>
                        <a href={`https://blockchair.com/search?q=${p.tx_hash}`} target="_blank" rel="noreferrer" style={{ color: '#2d8cf0', textDecoration: 'none' }}>
                          {p.tx_hash} ↗
                        </a>
                      </td>
                  ...
                  {pastPayments.map((p) => (
                    <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', opacity: 0.7 }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <strong>{p.prospect?.email || 'Unknown'}</strong>
                      </td>
                      <td style={{ padding: '12px 16px', fontFamily: 'monospace', fontSize: '11px', wordBreak: 'break-all' }}>{p.tx_hash}</td>
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{
